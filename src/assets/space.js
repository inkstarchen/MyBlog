(() => {
  ...
  async function closeReadingMode(animated = false) {
    ...
    await new Promise((resolve) => window.setTimeout(resolve, 790));
    document.body.classList.remove("reading-leaving");
    contentPanel.classList.remove("is-closing");
    readingScrollbar.classList.remove("is-closing");
  }
  ...
  async function navigate(path, options = {}) {
    ...
    transitioning = true;
    if (wasReading) document.body.classList.add("orbit-held");
    await closeReadingMode(true);
    if (wasReading && !prefersReducedMotion()) {
      await new Promise((resolve) => window.setTimeout(resolve, 820));
    }
    const oldParticles = [...particles];
    const duration = prefersReducedMotion() || wasReading ? 1 : 560;
